    ...
    $.toJSON = typeof JSON === "object" && JSON.stringify ? JSON.stringify : function(o) {
        ...
        if (type === "object") {
            ...
            for (var k in o) { type = typeof k;
                if (type === "number") { name = '"' + k + '"' } else {
                    if (type === "string") { name = $.quoteString(k) } else {
                        continue } }
                type = typeof o[k];
...
function isMeta(b) {
    ...
    var c = b.which,
        a = b.button;
    if (!c && a !== undefined) {
        return (!a & 1) && (!a & 2) && (a & 4) } else {
        if (c === 2) {
            return true } else {
            if (a === 2) {
                return true } } }
    return false }
document.onclick = function(i) { i = i || event;
    if (!i.clientX && !i.clientY && !i.pageX && !i.pageY) {
        return }
    var w = document,
        b = window;
    var a = tag = i.srcElement || i.target;
    var m = $(tag).attr("clstag"),
        p = $(tag).attr("href");
    var u = "";
    while (!m) { tag = tag.parentNode;
        if (!tag || (tag.nodeName == "BODY")) {
            break }
        m = $(tag).attr("clstag");
        if (!p) { p = $(tag).attr("href");
            a = tag } }
    if (m) {
        var f = m.split("|"),
            c = f[1],
        ...
        if (c === "keycount" && JA) {
            var h = JA.util.Nt();
            if (p) { JA.tracker.aloading(r, v, ["Q", p]);
                JA.tracker.ngloader("other.000000", { t1: r, t2: v, p0: JA.util.join(["Q", p]), cb: h.jdcb }) } else { JA.tracker.aloading(r, v, ["Q"]);
                JA.tracker.ngloader("other.000000", { t1: r, t2: v, p0: JA.util.join(["Q"]), cb: h.jdcb }) }
            u = r + "|" + v;
            if (p && /^(http:\/\/|https:\/\/|\/\/).*/.test(p) && $(a).attr("target") !== "_blank" && !isMeta(i)) { i.preventDefault ? i.preventDefault() : i.returnValue = false;
                setTimeout(function() {
                    var d = w.createElement("a");
                    d.href = p;
                    d.target = "_self";
                    w.body.appendChild(d);
                    if (typeof d.click !== "undefined") { d.click() } else { b.location.href = p }
                    w.body.removeChild(d) }, 200) } } }
    var l = this.location.hostname.toLowerCase();
    if (/(sale|mall|jmall|pop).(jd|360buy).(com|hk)/.test(l) || b.ja_heat_map) {
        var q = 0,
            o = 0,
            g = b.screen.width >= 1210 && l == "item.jd.com" ? 1210 : 990,
            n = w.body.clientWidth > g ? Math.round((w.body.clientWidth - g) / 2) : 0;
        if (i.pageX || i.pageY) { q = i.pageX;
            o = i.pageY } else { q = i.clientX + w.body.scrollLeft - w.body.clientLeft;
            o = i.clientY + w.body.scrollTop - w.body.clientTop }
        log("d", "c", u || "-", q + "x" + o, w.body.scrollWidth + "x" + w.body.scrollHeight, n) } };

function HashMap() { this.values = new Object() }
HashMap.prototype.Set = function(a, b) { this.values[a] = b };
HashMap.prototype.Get = function(a) {
    return this.values[a] };
HashMap.prototype.Contains = function(a) {
    return this.values.hasOwnProperty(a) };
HashMap.prototype.Remove = function(a) { delete this.values[a] };
var SucInfoMethod = { Init: function() { this.orderDetailMap = new HashMap();
        this.rSM = new HashMap();
        var b = SucInfo_OrderDetail.toString().split(",");
        for (var c = 0; c < b.length; c++) {
            var a = b[c].split(":");
            this.orderDetailMap.Set(a[0], a[1]);
            this.sku = a[0] } }, GetSkuNum: function(a) {
        return this.orderDetailMap.Get(a) }, Contains: function(a) {
        return this.orderDetailMap.Contains(a) }, GetDefaultSku: function() {
        return this.sku }, ARS: function(a) { this.rSM.Set(a, 0) }, RSContains: function(a) {
...
function RecommendTrans(recName, tag, logtype) {
    ...
    for (var i = 0; i < cookieNames.length; i++) {
        ...
        for (var k in recCookies) {
            if (recCookies[k] != "") {
                if (k == "cai2012") { loginfo(recCookies[k], k.toString(), "R", logtype) } else { loginfo(recCookies[k], k.toString(), tag, logtype) } } } } }

function simpleMold(d, a, g, f, b) {
    for (var e = 0; e < d.length; e++) {
        var c = getCookie(g + d[e]);
        if (c != null && c != "") { loginfo(c, d[e], a, f, b) } } }
...
function complexMold(cookieArrary, tag, prefix, logtype, flag) {
    for (var i = 0; i < cookieArrary.length; i++) {
        ...
        if (items != null) {
            for (var k in items) {
                if (items[k] != "") { loginfo(items[k], k.toString(), tag, logtype, flag) } } } } }
    ...
    for (var d = 0; d < g.length; d++) {
        if (g[d].length > 0) {
            ...
            if (SucInfoMethod.Contains(l)) {
                if (h) { log(e, a, j.concat(".o"), c, f, b, l + ":" + SucInfoMethod.GetSkuNum(l));
                    log("4", "R" + j.concat(".o"), c, f, b, l, SucInfoMethod.GetSkuNum(l)) } else { log(e, a + j, c, f, b, l, SucInfoMethod.GetSkuNum(l)) } } } } }

function isChecked() { SucInfo_OrderId = window.SucInfo_OrderId || JA.util.getParameter(document.location.href, "suc_orderid") || undefined;
...
function funLoad() {
    var a = getCookie("pin");
    if (a != null && a.length > 0) { setCookieMills("rpin", a, 259200000) } }

function Clublog() {
    var b = this.location.pathname.toLowerCase();
    var a = this.location.hostname.toLowerCase();
    if ((b.indexOf("/cart.html", 0) >= 0) || (b.indexOf("shoppingcart", 0) >= 0)) { log("R2&Page", "Show") } else {
        if (b.indexOf("user_home", 0) >= 0) { log("R3&Page", "Show") } else {
            if ((b.indexOf("initcart.html", 0) >= 0) || (b.indexOf("addtocart.html", 0) >= 0) || (b.indexOf("initcart.aspx", 0) >= 0)) { log("R4R5&Page", "Show") } else {
                if ((b.indexOf("normal/list.action", 0) >= 0) || (b.indexOf("orderlist.aspx", 0) >= 0)) { log("DDR&Page", "Show") } else {
                    if (a == "home.360buy.com") {
                        if (b == "/") { log("R3&Page", "Show") } } } } } } }
...
function getHistory() {
    ...
    if (d == null && b != null) {
        ...
        $.ajax({ url: a, type: "GET", dataType: "jsonp", success: function(e) {
                ...
                if (f.toString().length > 0) { setCookieMills("_ghis", f.toString().substring(0, 51)) }
                if (g.toString().length > 0) { setCookieMills("_ghit", g) } } }) } }(function() {
    function HashMap() { this.values = new Object() }
    ...
    function SortedHashMap(IComparer, IGetKey) { this.IComparer = IComparer;
    ...
    SortedHashMap.prototype.Insert = function(value, maxlength) {
        for (var i = 0, l = this.a.length; i < l; i++) {
            if (this.a[i].s === value.s) { this.a.splice(i, 1);
                break } }
        if (this.a.length >= maxlength) { this.a.splice(maxlength - 1, 1) }
    ...
    SortedHashMap.prototype.Clear = function() { this.a = new Array();
        this.h = new HashMap() };
    SortedHashMap.prototype.GetJson = function() {
        return $.toJSON(this.a) };

    function ThirdType(thirdType, sku, value) { this.t = thirdType;
        this.v = 5;
        this.s = 0;
        if (arguments.length > 1) { this.s = sku }
        if (arguments.length > 2) { this.v = value } }
    ThirdType.prototype.Increase = function() { this.v = this.v + 2 };
    ...
    Ttracker = { IComparer: function(a, b) {
            return b.v - a.v }, IGetKey: function(a) {
            return a.t }, isbook: function(id) {
            return id > 10000000 && id < 20000000 }, trace: function() {
            ...
            if (maps) { Ttracker.util.Wv("typewid", "", -63072000000) } }, viewhisotry: function(t, s, cname) {
            var nview = { t: t, s: s };
                ...
                try {
                    if (bview.indexOf(".") > 0) {
                        ...
                        for (var j = viewarray.length - 1; j >= 0; j--) {
                            ...
                            bookmap.Insert({ t: Number(book[0]), s: Number(book[1]) }, 8) } } else {
                        var bviews = eval("(" + bview + ")");
                        if (bviews.length > 0 && bviews[0].d != undefined) { Ttracker.util.Wv(cname, "", -63072000000) } else {
                            for (var i = bviews.length - 1; i >= 0; i--) { bookmap.Insert(bviews[i], 8) } } } } catch (e) { Ttracker.util.Wv(cname, "", -63072000000) } }
            bookmap.Insert(nview, 8);
            var cvalue = "";
            for (var k = 0, klen = bookmap.a.length; k < klen; k++) { cvalue += (bookmap.a[k].t + "." + bookmap.a[k].s + (k == klen - 1 ? "" : "|")) }
                ...
                try {
                    if (vrate.indexOf(".") > 0) {
                        ...
                        for (var j = ratearray.length - 1; j >= 0; j--) {
                            ...
                            var tv = Number(tw[2] || 0),
                                tid = Number(tw[0]);
                            tv = t === tid ? tv : (tv - 1);
                            sitesortmap.Add(Number(tw[0]), { t: Number(tw[0]), s: Number(tw[1]), v: tv }, 8) } } else {
                        var vrates = eval("(" + vrate + ")");
                        if (vrates.length > 0 && vrates[0].d != undefined) { Ttracker.util.Wv(cname, "", -63072000000) } else {
                            for (var i = 0; i < vrates.length; i++) {
                                var rate = vrates[i];
                                if (rate.t != t) { rate.v -= 1 }
                                sitesortmap.Add(rate.t, rate) } } } } catch (e) { Ttracker.util.Wv(cname, "", -63072000000) } }
            if (!sitesortmap.ContainsKey(t)) { sitesortmap.Add(t, ntw) } else {
            ...
            var cvalue = "";
            for (var k = 0, klen = sitesortmap.a.length; k < klen; k++) { cvalue += (sitesortmap.a[k].t + "." + sitesortmap.a[k].s + "." + sitesortmap.a[k].v + (k == klen - 1 ? "" : "|")) }
            cvalue && Ttracker.util.Wv(cname, cvalue, 63072000000) }, view: function(t, s) {
            var tid = Number(t),
                sku = Number(s),
                _this = this;
            $.ajax({ url: "//diviner.jd.com/cookie?ck=" + tid + "." + sku, dataType: "jsonp", success: function(json) {
                    if (typeof(json) == "object" && json.errCode == 0) { _this.util.Wv("atw", "", -63072000000);
                        if (_this.isbook(sku)) { _this.util.Wv("btw", "", -63072000000);
                            _this.util.Wv("bview", "", -63072000000) } } } });
            $.ajax({ url: "//x.jd.com/aview?ck=" + tid + "." + sku, dataType: "jsonp", success: function(res) {
                    if (typeof(res) == "object" && res.errCode == 0) { _this.util.Wv("aview", "", -63072000000) } } }) } };
    Ttracker.util = { Wv: function(n, v, t) {
            ...
            n += "domain=" + d + ";";
            document.cookie = n }, Vv: function(n) {
...
(function() {
    ...
    r.util = { join: function(l) {
            if (l instanceof Array) {
                var s = "";
                for (var p = 0, g = l.length; p < g; p++) { s += l[p] + ((p == g - 1) ? "" : "|||") }
                return s }
            return l }, getParameter: function(p, l) {
            var s = new RegExp("(?:^|&|[?]|[/])" + l + "=([^&]*)");
            var g = s.exec(p);
            return g ? aD(g[1]) : "" }, Wv: function(s, g, p, l) { s = s + "=" + g + "; path=/; ";
            l && (s += "expires=" + (new Date(new Date().getTime() + l)).toGMTString() + "; ");
            p && (s += "domain=" + p + ";");
            ap.cookie = s }, Vv: function(y) {
    ...
    var aR = function() {
        ...
        var p = "",
            y;
        if ((y = r.util.Vv("pinId")) && y[R]) { p = y[0] }
        this.set(al, p || "-") };
    var ao = "baidu:wd,baidu:word,haosou.com:q,so.com:q,so.360.cn:q,360so.com:q,360sou.com:q,baidu:q1,m.baidu:word,m.baidu:w,wap.soso:key,m.so:q,page.yicha:key,sz.roboo:q,i.easou:q,wap.sogou:keyword,google:q,soso:w,sogou:query,youdao:q,ucweb:keyword,ucweb:word,114so:kw,yahoo:p,yahoo:q,live:q,msn:q,bing:q,aol:query,aol:q,daum:q,eniro:search_word,naver:query,pchome:q,images.google:q,lycos:query,ask:q,netscape:query,cnn:query,about:terms,mamma:q,voila:rdata,virgilio:qs,alice:qs,yandex:text,najdi:q,seznam:q,search:q,wp:szukaj,onet:qt,szukacz:q,yam:k,kvasir:q,ozu:q,terra:query,rambler:query".split(","),
        ...
        U = function() {
            ...
            if ((D = (D = window.navigator) ? D.plugins : S) && D[R] > 0) {
                for (C = 0; C < D[R] && !t; C++) { y = D[C], y.name[w]("Shockwave Flash") > -1 && (t = y.description[M]("Shockwave Flash ")[1]) } } else { y = y + "." + y;
                try { C = new ActiveXObject(y + ".7"), t = C.GetVariable("$version") } catch (s) {}
                if (!t) {
                    try { C = new ActiveXObject(y + ".6"), t = "WIN 6,0,21,0", C.AllowScriptAccess = "always", t = C.GetVariable("$version") } catch (p) {} }
                if (!t) {
                    try { C = new ActiveXObject(y), t = C.GetVariable("$version") } catch (l) {} }
        ...
        X = function(l) {
            var g = 1,
                ...
                p;
            if (!at(l)) { g = 0;
                for (p = l[R] - 1; p >= 0; p--) { s = l.charCodeAt(p), g = (g << 6 & 268435455) + s + (s << 14), s = g & 266338304, g = s != 0 ? g ^ s >> 21 : g } }
            return g },
        an = function() {
            var p = aL();
            for (var l = p, g = ad.navigator, l = g.appName + g.version + l.language + g.platform + g.userAgent + l.javaEnabled + l.D + l.C + (ap.cookie ? ap.cookie : "") + (ap.referrer ? ap.referrer : ""), g = l.length, s = ad.history.length; s > 0;) { l += s-- ^ g++ }
            return X(l) },
        ac = function() {
            var g = { name: "other", version: "0" },
        ...
        aJ = function() {
            ...
            for (var t = 0, g = y.length; t < g; t++) {
                ...
                if (s[R] == 0) {
                    continue }
                var C = ae(s[0]).match(/=(.*?)&/gi),
                    l = [];
                if (C == null) {
                    continue }
                $.each(C, function(K, D) { l.push(K == 0 ? "T" + D.substring(1, D.length - 1) : D.substring(1, D.length - 1)) });
                p += l[F]("-") + ";" }
            return p },
        aH = function(t) { t.set(ag, ap.location.hostname);
            t.set(aj, ap.title.replace(/\$/g, ""));
            t.set(aA, ap.location.pathname);
            t.set(az, ap.referrer.replace(/\$/g, ""));
            t.set(aI, ap.location.href);
            var g = r.util.Vv("__jda"),
                L = g[R] > 0 ? g[0][M](".") : null;
        ...
        aG = function(a2) {
            var s = ap.location.search,
                ...
                O = a2.get(av),
                L = a2.get(c),
                g = r.util.Vv("__jdb")[R] == 0;
            var a3 = false;
            if (y) {
                var l = r.util.getParameter(s, "utm_campaign"),
                    a1 = r.util.getParameter(s, "utm_medium"),
                ...
                if (p && p[w](aZ) < 0) {
                    for (var ak = a2.get(aN), aX = 0; aX < ak.length; aX++) {
                        ...
                        if (p[w](aY[0][H]()) > -1 && C[w]((aY[1] + "=")[H]()) > -1) {
                            ...
                            break } }
                    if (!a0) {
                        if (p[w]("zol.com.cn") > -1) { t[P]("zol.com.cn");
                            t[P]("-");
                            t[P]("cpc");
                            t[P]("not set") } else { t[P](p);
                            t[P]("-");
                            t[P]("referral");
                            t[P]("-") } } } }
            var D = t[R] > 0 && (t[0] !== W || t[1] !== O || t[2] !== L) && t[2] !== "referral";
            if (g || (!g && D)) { a2.set(f, t[0] || a2.get(f));
    ...
    var q = new aR(),
        aw = function() { this.a = {};
            this.add = function(g, l) { this.a[g] = l };
            this.get = function(g) {
                return this.a[g] };
            this.toString = function() {
                return this.a[F]("&") } },
        o = function(l, g) { g.add("jdac", l.get(am)), g.add("jduid", l.get(af)), g.add("jdsid", l.get(af) + "|" + l.get(ax)), g.add("jdje", l.get(n)), g.add("jdsc", l.get(N)), g.add("jdsr", l.get(z)), g.add("jdul", l.get(aC)), g.add("jdcs", l.get(A)), g.add("jddt", l.get(aj) || "-"), g.add("jdmr", aD(l.get(az))), g.add("jdhn", l.get(ag) || "-"), g.add("jdfl", l.get(aS)), g.add("jdos", l.get(B)), g.add("jdbr", l.get(ab)), g.add("jdbv", l.get(V)), g.add("jdwb", l.get(i)), g.add("jdxb", l.get(a)), g.add("jdyb", l.get(aP)), g.add("jdzb", l.get(ax)), g.add("jdcb", l.get(Q)), g.add("jdusc", l.get(f) || "direct"), g.add("jducp", l.get(av) || "-"), g.add("jdumd", l.get(c) || "-"), g.add("jduct", l.get(ar) || "-"), g.add("jdlt", typeof jdpts != "object" ? 0 : jdpts._st == undefined ? 0 : l.get(aW) - jdpts._st), g.add("jdtad", l.get(b)), g.add("jdak", l.get(ay)), g.add("pinid", l.get(al)) },
        aU = function(l, g, p, s) { g.add("jdac", l.get(am)), g.add("jduid", l.get(af)), g.add("jdsid", l.get(af) + "|" + l.get(ax)), g.add("jdje", "-"), g.add("jdsc", "-"), g.add("jdsr", "-"), g.add("jdul", "-"), g.add("jdcs", "-"), g.add("jddt", "-"), g.add("jdmr", aD(l.get(az))), g.add("jdhn", "-"), g.add("jdfl", "-"), g.add("jdos", "-"), g.add("jdbr", "-"), g.add("jdbv", "-"), g.add("jdwb", "-"), g.add("jdxb", "-"), g.add("jdyb", "-"), g.add("jdzb", l.get(ax)), g.add("jdcb", s ? (aF() + s) : l.get(Q)), g.add("jdusc", "-"), g.add("jducp", "-"), g.add("jdumd", "-"), g.add("jduct", "-"), g.add("jdlt", 0), g.add("jdtad", p), g.add("jdak", l.get(ay)), g.add("pinid", l.get(al)) },
        aT = function() { aH(q);
            var p = aG(q),
                g = r.util.Vv("__jdv");
            var s = new aw(),
            ...
            if (p || !g.length) { r.util.Wv("__jdv", x(q), l, q.get(Y)) }
        ...
        aE = function(l) {
            var g = J(l);
            g.onload = g.onerror = function() { g.onload = null;
                g.onerror = null } };
    r.util.Nt = aB;
    r.tracker = { sendOld: function(s, p, l, t) {
            return;
            ...
            $.getJSON(g, function() {}) }, sendNew: function(l, s) {
            var p = aB();
            ...
            aE(g) }, ngloader: function(g, p) {
            var l = aB();
            var s = { je: l.jdje, sc: l.jdsc, sr: l.jdsr, ul: l.jdul, cs: l.jdcs, dt: l.jddt, hn: l.jdhn, fl: l.jdfl, os: l.jdos, br: l.jdbr, bv: l.jdbv, wb: l.jdwb, xb: l.jdxb, yb: l.jdyb, zb: l.jdzb, cb: l.jdcb, usc: l.jdusc, ucp: l.jducp, umd: l.jdumd, uct: l.jduct, ct: new Date().getTime(), lt: l.jdlt, tad: l.jdtad };
            this.ngaloader(g, s, p) }, ngaloader: function(l, y, p) {
            var s = "";
            for (var g in y) { s += ((g + "=" + y[g]) + "$") }
            if (p) {
                for (var g in p) { s += ((g + "=" + p[g]) + "$") } }
            s += "pinid=" + q.get(al) + "$";
            s = s.substring(0, s.length - 1);
            this.sendNew(l, s) }, ngloaderJSON: function(g, p) {
            var l = aB();
            p.pinid = q.get(al);
            p.je = l.jdje;
            p.sc = l.jdsc;
            ...
            p.lt = l.jdlt;
            p.tad = l.jdtad;
            this.sendNew(g, $.toJSON(p)) }, bloading: function(p, g, s) {
            var l = aT();
            ...
            (l.jduid % 1000) === 1 && this.ngloader("jsver.000000", { jsfile: "wl", jsver: "20141223" }) }, loading: function(p, l, g, s) { this.sendOld(p, l, g, JA.util.join(s)) }, aloading: function(p, l, s) {
            var g = aB();
            this.loading(p, l, g, s) }, aloadingJSON: function(p, l, s) {
            var g = aB();
            this.sendOld(p, l, g, $.toJSON(s)) }, adshow: function(l) {
            var g = aO(l);
            this.loading("AD", "IM", g, "") }, adclick: function(l) {
            var g = aO(l, 1);
    ...
    if ((window.pageConfig && window.pageConfig.product && window.pageConfig.product.cat) || T) { r.tracker.ngloader("item.010001", { sku: T[4] || window.pageConfig.product.skuid, cid1: T[1] || window.pageConfig.product.cat[0], cid2: T[2] || window.pageConfig.product.cat[1], cid3: T[3] || window.pageConfig.product.cat[2], brand: T ? "0" : window.pageConfig.product.brand }) }(function() {
        if (isChecked()) { SucInfoMethod.Init();
            ...
            for (var s = 0; s < g.length; s++) {
                var D = getCookie(g[s]);
                if (D != null && D != "") { log("HomePageOrder", g[s]) } }
            ...
            var t = aB();
            JA && JA.tracker.ngloader("order.100000", { orderid: SucInfo_OrderId, ordertype: SucInfo_OrderType, orderdetail: SucInfo_OrderDetail, cb: t.jdcb }) } })() })();
...
function logJSON(b, a, d) {
    ...
    JA.tracker.ngloaderJSON("other.000000", { t1: b, t2: a, p0: d }) }(function() {
    if (typeof jdpts != "object") {
        return }
    if (jdpts._cls) { log(jdpts._cls.split(".")[0], jdpts._cls.split(".")[1]) } })();
Clublog();
